...
export default function SiteHeader({ showAnchorLinks = false, breadcrumb }: SiteHeaderProps) {
    ...
    <>
      <nav
        className={`fixed top-0 left-0 right-0 z-50 border-b border-border/50 backdrop-blur-md transition-all duration-300 ${
          scrolled ? "bg-background/95 shadow-lg shadow-black/20" : "bg-background/80"
        }`}
      >
        <div className="container flex items-center justify-between h-16">
          {/* Logo */}
          <div className="flex items-center gap-3 min-w-0">
            <Link href="/" className="flex items-center gap-2.5 group flex-shrink-0" aria-label="BlankAI Home">
          ...
          <div className="hidden md:flex items-center gap-1">
            {showAnchorLinks && anchorLinks.map((link) => (
              <a
                key={link.label}
                href={link.href}
        ...
        <div className="container py-4 space-y-1">
          ...
          {showAnchorLinks && (
            <div className="pb-2 mb-2 border-b border-border/50">
              <p className="text-[10px] font-mono-custom text-muted-foreground/50 uppercase tracking-wider px-3 mb-2">Navigation</p>
          ...
          {/* Tools */}
          <div className="pb-2">
            <p className="text-[10px] font-mono-custom text-muted-foreground/50 uppercase tracking-wider px-3 mb-2">Tools</p>
            <Link
              href="/"
              onClick={() => setOpen(false)}
              className={`flex items-center justify-between px-3 py-2.5 rounded-lg transition-all text-sm ${
                isHome ? "text-cyan bg-cyan/10" : "text-muted-foreground hover:text-foreground hover:bg-muted/50"
              }`}
            >
              <div className="flex items-center gap-2">
              ...
              {isHome && <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-cyan/20 text-cyan font-bold">ACTIVE</span>}
            </Link>
            <Link
              href="/image-diff"
              onClick={() => setOpen(false)}
              className={`flex items-center justify-between px-3 py-2.5 rounded-lg transition-all text-sm ${
                location === "/image-diff" ? "text-cyan bg-cyan/10" : "text-muted-foreground hover:text-foreground hover:bg-muted/50"
              }`}
            >
              <div className="flex items-center gap-2">
                <svg className="w-4 h-4" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5">
            ...
            <Link
              ...
              onClick={() => setOpen(false)}
              className={`flex items-center justify-between px-3 py-2.5 rounded-lg transition-all text-sm ${
                location.startsWith("/blog") ? "text-cyan bg-cyan/10" : "text-muted-foreground hover:text-foreground hover:bg-muted/50"
              }`}
            >
              <div className="flex items-center gap-2">
                <svg className="w-4 h-4" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5">
              ...
              {location.startsWith("/blog") && <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-cyan/20 text-cyan font-bold">ACTIVE</span>}
            </Link>
          </div>
          ...
          <div className="pt-2 border-t border-border/50">
            <div className="flex gap-4 px-3 text-xs text-muted-foreground">
              <Link href="/privacy" onClick={() => setOpen(false)} className="hover:text-foreground transition-colors">Privacy</Link>
              <Link href="/terms" onClick={() => setOpen(false)} className="hover:text-foreground transition-colors">Terms</Link>
              <a href="mailto:support@blankai.app" className="hover:text-cyan transition-colors">support@blankai.app</a>
            </div>
          </div>
        </div>
      </div>
