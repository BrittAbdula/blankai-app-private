...
import type { ExifResult, MetaField, MetaGroup } from "@blankai-core/lib/exifReader";
import { convertHeicToJpegDataUrl } from "@extension/lib/heicSandbox";
import { isHeicLike } from "@extension/lib/imagePreview";
...
export const EDITABLE_FIELD_DEFINITIONS: EditableFieldDefinition[] = [
  ...
  },
  {
    ...
    helperText: "Leave latitude and longitude both empty to remove GPS.",
  },
  {
    key: "longitude",
...
const DEFINITIONS_BY_GROUP = EDITABLE_FIELD_DEFINITIONS.reduce<Record<string, EditableFieldDefinition[]>>(
  (acc, definition) => {
...
function formatDateForInput(date: Date, offset: string) {
  ...
  return (
    [adjusted.getUTCFullYear(), pad(adjusted.getUTCMonth() + 1), pad(adjusted.getUTCDate())].join("-") +
    `T${pad(adjusted.getUTCHours())}:${pad(adjusted.getUTCMinutes())}:${pad(adjusted.getUTCSeconds())}`
  );
}
...
function parseDateSource(value: unknown) {
  ...
  if (typeof value === "string") {
    const normalized = value.includes(":") ? value.replace(/^(\d{4}):(\d{2}):(\d{2})\s/, "$1-$2-$3T") : value;
...
async function loadRenderableImage(file: File) {
  ...
  try {
    return await new Promise<HTMLImageElement>((resolve, reject) => {
      ...
      image.onerror = () => reject(new Error("This image format cannot be rendered to a new JPEG in this browser."));
      image.src = objectUrl;
    });
  } finally {
...
async function renderFileToJpegDataUrl(file: File, quality = 0.95) {
  if (isHeicLike(file)) {
    return await convertHeicToJpegDataUrl(file, quality);
  }
...
function parseGpsAltitude(rawAll: Record<string, unknown>) {
  const rawAltitude = firstValue(rawAll.GPSAltitude, rawAll.altitude, rawAll.Altitude);
...
export function buildMetadataEditDraft(result: ExifResult): MetadataEditDraft {
  ...
  const utcOffset = firstText(rawAll.OffsetTimeOriginal, rawAll.OffsetTime, rawAll.OffsetTimeDigitized);
  ...
  return {
    ...
    dateTaken: toDateTimeInput(firstValue(rawAll.DateTimeOriginal, rawAll.CreateDate), utcOffset),
    dateDigitized: toDateTimeInput(rawAll.DateTimeDigitized, utcOffset),
    dateModified: toDateTimeInput(firstValue(rawAll.DateTime, rawAll.ModifyDate), utcOffset),
    utcOffset,
...
function findEditableDefinition(field: MetaField) {
  return EDITABLE_FIELD_DEFINITIONS.find((definition) => definition.matchKeys.includes(field.key));
}
...
function mergeEditableFields(group: MetaGroup, draft: MetadataEditDraft): DisplayMetaGroup {
  ...
  for (const definition of definitions) {
    const alreadyPresent = fields.some(
      (field) => definition.matchKeys.includes(field.key) || field.key === definition.fieldKey,
    );
    if (!alreadyPresent) {
